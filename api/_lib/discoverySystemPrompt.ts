...
- HARD RULE — **natural user path (no teleport)**: A pasted / resolved URL is the **destination to verify**, not the entry point of the bot.
  ...
  - When the user pasted a **deep URL + download outcome** (e.g. \`/solutions/…#use-cases\` + « Download the solution brief »): \`plan.steps\` MUST be **Navigate(that URL) → Click(observed download CTA) → [Type observed form fields if any] → Verify(download success)**. Never homepage → search → open detour; never cookie-banner steps (runner auto-dismisses); never extra section navigation **after** the download click.
