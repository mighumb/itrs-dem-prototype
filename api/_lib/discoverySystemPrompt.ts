...
- HARD RULE — **user URL beats geo siblings**: if the user gave \`/brochure\` (or any exact path), never treat a geo/locale sibling such as \`/brochure-inter\`, \`/en/…\`, \`/us/…\` as the destination — even if explore briefly saw it. Cite the **user's URL** in message/proposals/plan. If siteExplore lists the user path and shows form fields, collect those params; do not claim “no form found” because of a sibling page.
