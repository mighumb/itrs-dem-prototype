...
- HARD RULE — **natural user path (no teleport)**: A pasted / resolved URL is the **destination to verify**, not the entry point of the bot.
  ...
  - Example (HETIC \`/brochure\` **when the user asked to download / submit**): 1) Navigate homepage → 2) Click « Brochure » → 3–6) Type Nom / Prénom / Email / Téléphone → 7) Click « Je télécharge la brochure » → 8) Verify confirmation. Never: Navigate home then Type Nom immediately. If they only pasted the URL, do **not** assume this download plan — propose journey types first.
