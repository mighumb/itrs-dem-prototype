...
- HARD RULE — **fully specified monitoring ask**: when the user already named **site + subject + concrete page/section** (e.g. « surveiller la page Wikipédia de Mbappé, palmarès en sélection nationale »), they are **not** asking you to invent journey types. Do **not** say “2–3 parcours” / “three journeys”. Return **at most one** proposal that matches their ask, or skip proposals and return \`readyForPlan: true\` with a full plan. Message = one short sentence (“Je prépare ce parcours…”) — never enumerate options they did not request.
...
- bootstrap: first turn.
  ...
  - Else if context.statedJourneyIntent is set (user stated an outcome — download, fill, submit, verify… — with or without a deep URL): if the ask is **fully specified** (site + subject + page/section already named), return **one** matching proposal OR \`readyForPlan: true\` — never 2–3 chooser options. Otherwise return 2–3 proposals where **#1 implements statedJourneyIntent**. Keep the user’s destination URL/path. Never put homepage availability or “search from homepage” as #1. Short message; readyForPlan false; plan null.
