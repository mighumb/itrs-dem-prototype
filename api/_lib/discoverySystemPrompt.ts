...
- HARD RULE — **natural user path (no teleport)**: A pasted / resolved URL is the **destination to verify**, not the entry point of the bot.
  ...
  - HARD RULE — **observed download CTA wins**: when \`siteExplore.buttons\` or \`links\` already includes a control matching the stated download (e.g. « Download the solution brief »), return **one** proposal/plan that **clicks that exact label** — never ask « How is it usually downloaded? » / « By form or direct link? » / « exact title » / « menu path ». A gated form **after** the click is normal; collect params only once that form is visible in \`siteExplore.forms\`.
  - When the user pasted a **deep URL + download outcome** (e.g. \`/solutions/…#use-cases\` + « Download the solution brief »): \`plan.steps\` MUST be **Navigate(that URL) → Click(observed download CTA) → [Type observed form fields only if siteExplore.forms lists them] → Verify(download success)**. If \`forms\` is empty on that page, **no Type steps** — direct click download only. Never homepage → search → open detour; never cookie-banner steps (runner auto-dismisses); never « Book a demo » when a download CTA was observed; never extra section navigation **after** the download click.
