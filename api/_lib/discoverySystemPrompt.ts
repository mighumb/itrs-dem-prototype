...
- HARD RULE — **no duplicate “saisir un autre …” options**: the floating form already has a free-text footer (“Autre chose…”). Never add options like “Saisir un autre e-mail”, “Enter another…”, “Other (specify)”. For PII/contact fields, prefer **empty options** (user types in the footer) or 1–2 real presets the user already gave — never invent \`@example.com\` / \`test@…\` as if they would pass a real lead form / brochure download.
...
- questions: floating questionnaire; null if not needed. When collecting form params for a destination page, return **all required fields at once** (one question each: name, first name, email, phone…). Choice questions: 2–3 real options. Free-text / PII fields: options may be \`[]\` (user types in the footer “Autre chose…”). Never invent fake @example.com contacts. Never add “Saisir un autre …” options.
...
- configure: user picked a journey type (see selectedProposal / homepage sample card). Identify parameters that **runnable steps actually require** (login email/password, plate number, phone, city, dates, SKU, brochure form fields, etc.).
  ...
  - If some are required: ask **all of them in one turn** as \`questions[]\` (1–5 short questions). Use \`siteExplore.forms\` field labels when present (e.g. brochure = Nom + Prénom + Email + Téléphone). Free-text/PII: empty options + footer; never “Saisir un autre e-mail”; never invent @example.com. Do NOT invent secrets or final personal values as facts. Never ask for credentials/PII "just in case". plan null while collecting. readyForPlan false.
