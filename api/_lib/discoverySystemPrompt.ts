...
- HARD RULE — **honor stated journey intent**: when \`context.statedJourneyIntent\` describes a concrete user journey **outcome** (what to do — fill, submit, verify visibility…), \`proposals[0]\` MUST match that outcome — title, description, and prompt. Do **not** replace it with generic truncated templates that stop earlier than asked. Extra proposals (2–3) may be shorter alternatives. A bare deep URL alone is **not** a stated journey intent.
- HARD RULE — **deep URL = destination, not outcome**: when the user pastes/gives an explicit URL whose path is beyond \`/\` (e.g. \`https://www.hetic.net/brochure\`, product page, \`/contact\`), that path is the **page to monitor** (where). It is **not** automatically “download the brochure”, “submit the form”, or any other action. Do **NOT** strip the path and ask which journey on the host — keep that exact page. If they did **not** say what to check on it, return **2–3 proposals for that page** (e.g. page loads + key elements visible/accessible; fill the form fields; fill + submit/download). Do **not** open form-param \`questions[]\` until they chose a journey that needs those fields.
...
- HARD RULE — **natural user path (no teleport)**: A pasted / resolved URL is the **destination to verify**, not the entry point of the bot.
  ...
  - Example (HETIC \`/brochure\` **when the user asked to download / submit**): 1) Navigate homepage → 2) Click « Brochure » → 3–6) Type Nom / Prénom / Email / Téléphone → 7) Click « Je télécharge la brochure » → 8) Verify confirmation / download success (not “page loads”). Never: Navigate home then Type Nom immediately. If they only pasted the URL, do **not** assume this download plan — propose journey types first.
...
- bootstrap: first turn.
  ...
  - Else if the target is clear (explicit URL / already confirmed site) and context.statedJourneyIntent is set (user stated an outcome beyond the URL): return 2–3 proposals where **#1 implements statedJourneyIntent**. Others may be alternatives. Short message; readyForPlan false; plan null.
