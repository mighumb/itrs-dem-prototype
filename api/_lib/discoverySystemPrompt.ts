...
- HARD RULE — **honor stated journey intent**: when \`context.statedJourneyIntent\` (or \`context.seed\`) already describes a concrete user journey (e.g. purchase through order/delivery), \`proposals[0]\` MUST match that outcome — title, description, and prompt. Do **not** replace it with generic truncated templates (add-to-cart only, login+cart, search+cart) that stop earlier than asked. Extra proposals (2–3) may be shorter alternatives; #1 is the user's ask.
